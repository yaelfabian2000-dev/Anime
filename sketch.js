let classifier;
let imageModelURL = 'https://teachablemachine.withgoogle.com/models/_LjCH64op/'; 
let video;
...
function draw() {
  ...
  if (label === "Anime" && confidence > 0.85) {
    noFill();
  ...
  if (label === "Anime" && confidence > 0.95) {
...
function drawCartoonUI() {
  ...
  let msg = (label === "Anime") ? "💥 ¡CARTOON DETECTADO! 💥" : "🔍 BUSCANDO DIBUJOS...";
  text(msg, 20, height - 50);

  textAlign(RIGHT);
  // Mostramos el porcentaje de confianza
  text(floor(confidence * 100) + "% Anime", width - 20, height - 50);
}
